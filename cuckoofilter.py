...
class CuckooFilter(object):
    ...
    def new_modified_insert_addXorValue(self, fingerprint, value):
        """
        Insert an item into the filter.
    ...
    def new_modified_contains(self, fingerprint):
        """
        Check if the filter contains the item.
    ...
    def new_getHiddenItem(self, fingerprint):
        """
        get HiddenItem object according to item
        :param item: Item to check its presence in the filter.
        :return: HiddenItem object.
    ...
    def new_delete(self, fingerprint):
        i = self._get_index(fingerprint)
        j = self._get_alternate_index(i, fingerprint)
        if self.buckets[i].new_delete(fingerprint) \
                or self.buckets[j].new_delete(fingerprint):
            self.size -= 1
            return True
        return False
...
def new_antiCollisionInsert(cfList=[], index=0, fingerprint=0, value=0):
    if cfList[index].new_modified_insert_addXorValue(fingerprint, value) == 1:
        return 1
    return new_antiCollisionInsert(cfList, index + 1, fingerprint, value)
...
def new_antiCollisionFind(cfList=[], fingerprint=0):
    for i in range(len(cfList)):
        if cfList[i].new_modified_contains(fingerprint):
            return True
    return False


def antigetHiddenItem(cfList=[], item=''):
    for i in range(len(cfList)):
        if cfList[i].modified_contains(item):
            return cfList[i].getHiddenItem(item)


def new_antigetHiddenItem(cfList=[], fingerprint=0):
    for i in range(len(cfList)):
        if cfList[i].new_modified_contains(fingerprint):
            return cfList[i].new_getHiddenItem(fingerprint)


def new_antidelete(cfList=[], fingerprint=0):
    for i in range(len(cfList)):
        if cfList[i].new_delete(fingerprint):
            return True
    return False
